...
def add_salesinvoice(request):
  ...
  user_id = request.user.id
  Party=party.objects.filter(company=Company.id)
  item=ItemModel.objects.filter(company=Company.id)

  return render(request, 'company/add_salesinvoice.html',{'Party':Party,'item':item})

def get_party_details(request, party_name):
    try:
...
def item_details(request, item_id):
    try:
        item = ItemModel.objects.get(item_name=item_id)
        data = {
            'hsn': item.item_hsn,
...
def itemdata_vendor_credit(request):
    ...
    item = ItemModel.objects.get(item_name=id, company=Company)
    name=item.item_name
    rate = item.item_sale_price
    hsn = item.item_hsn
    place = Company.state
